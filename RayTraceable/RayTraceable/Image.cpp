...
void Image::resize(int width, int height)
{
    ...
    memset(m_pixels, 0, width*height*sizeof(Pixel)); // Sets memory (intialize everything to 0)
    m_width = width;
    m_height = height;
}
...
{
    glRasterPos2f(-1, -1 + 2*y / (float)m_height); // Specify position of next raster operation(2 short)
